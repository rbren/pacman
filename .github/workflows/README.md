...
This directory contains the GitHub Actions workflows for the Pacman project.
...
- **Environments**:
  - **Production**: `pacman` (main branch)
  - **Preview**: `pacman-{branch-name}` (PRs and feature branches)
...
- **Actions**:
  - Deletes the associated Fly.io app (`pacman-{branch-name}`)
...
- **Features**:
  ...
  - Safe filtering (only deletes `pacman-*` apps, never the main `pacman` app)
...
- **Main app**: `pacman`
- **Feature apps**: `pacman-{clean-branch-name}`
...
# Output: pacman-feature-add-new-component
```
